    ...
    for pred_bbx in pred_bbxes:
        for gt_bbx in gt_bbxes:
            if pred_bbx[0] == gt_bbx[0] and pred_bbx[5] == gt_bbx[5]:
                iou = box_iou(
                    pred_bbx[1:5].unsqueeze(dim=0), gt_bbx[1:5].unsqueeze(dim=0)
                ).item()
                if iou > threshold:
    ...
    for bs in range(batches):
        for cls in range(1, num_classes + 1):
            if (label[bs] == cls).sum() >= _LABEL2MASK_THRESHOL:
                ...
                pixel_direction = centermap[bs, (cls - 1) * 3 : cls * 3][
                    :2, label[bs] == cls
                ]
                y_index = x_index.unsqueeze(dim=0) - pixel_location[0].unsqueeze(dim=1)
                y_index = torch.round(
                    pixel_location[1].unsqueeze(dim=1)
                    + (pixel_direction[1] / pixel_direction[0]).unsqueeze(dim=1)
                    * y_index
                ).to(torch.int32)
                mask = (y_index >= 0) * (y_index < H)
                count = y_index * W + x_index.unsqueeze(dim=0)
                center, inlier_num = (
                    torch.bincount(count[mask]).argmax(),
                    torch.bincount(count[mask]).max(),
                )
                center_x, center_y = (
                    center % W,
                    torch.div(center, W, rounding_mode="trunc"),
                )
                if inlier_num > _HOUGHVOTING_NUM_INLIER:
                    centers[bs, cls - 1, 0], centers[bs, cls - 1, 1] = (
                        center_x,
                        center_y,
                    )
                    xyplane_dis = xy - torch.tensor([center_x, center_y])[
                        :, None, None
                    ].to(device=label.device)
                    xyplane_direction = (
                        xyplane_dis / (xyplane_dis**2).sum(dim=0).sqrt()[None, :, :]
                    )
                    predict_direction = centermap[bs, (cls - 1) * 3 : cls * 3][:2]
                    inlier_mask = (
                        (xyplane_direction * predict_direction).sum(dim=0).abs()
                        >= _HOUGHVOTING_DIRECTION_INLIER
                    ) * label[bs] == cls
                    depths[bs, cls - 1] = centermap[bs, (cls - 1) * 3 : cls * 3][
                        2, inlier_mask
                    ].mean()
