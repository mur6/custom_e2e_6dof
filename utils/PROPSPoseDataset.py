...
class PROPSPoseDataset(Dataset):
    ...
    def parse_dir(self):
        ...
        assert (
            len(rgb_list) == len(depth_list) == len(scene_gt) == len(scene_gt_info)
        ), "data files number mismatching"
        all_lst = []
    ...
    def parse_model(self):
        ...
        objpathdict = {
            1: [
                "master_chef_can",
                os.path.join(model_path, "1_master_chef_can", "textured_simple.obj"),
            ],
            2: [
                "cracker_box",
                os.path.join(model_path, "2_cracker_box", "textured_simple.obj"),
            ],
            3: [
                "sugar_box",
                os.path.join(model_path, "3_sugar_box", "textured_simple.obj"),
            ],
            4: [
                "tomato_soup_can",
                os.path.join(model_path, "4_tomato_soup_can", "textured_simple.obj"),
            ],
            5: [
                "mustard_bottle",
                os.path.join(model_path, "5_mustard_bottle", "textured_simple.obj"),
            ],
            6: [
                "tuna_fish_can",
                os.path.join(model_path, "6_tuna_fish_can", "textured_simple.obj"),
            ],
            7: [
                "gelatin_box",
                os.path.join(model_path, "8_gelatin_box", "textured_simple.obj"),
            ],
            8: [
                "potted_meat_can",
                os.path.join(model_path, "9_potted_meat_can", "textured_simple.obj"),
            ],
            9: ["mug", os.path.join(model_path, "14_mug", "textured_simple.obj")],
            10: [
                "large_marker",
                os.path.join(model_path, "18_large_marker", "textured_simple.obj"),
            ],
        }
